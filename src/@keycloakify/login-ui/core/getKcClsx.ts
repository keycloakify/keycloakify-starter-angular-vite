...
import { clsx_withTransform } from 'keycloakify/tools/clsx_withTransform';
...
export function createGetKcClsx<ClassKey extends string>(params: {
  ...
  function areSameParams(params1: Param0<typeof getKcClsx>, params2: Param0<typeof getKcClsx>): boolean {
  ...
  function getKcClsx(params: { doUseDefaultCss: boolean; classes: Partial<Record<ClassKey, string>> | undefined }): {
    kcClsx: (...args: CxArg[]) => string;
  } {
    ...
    function kcClsx(...args: CxArg[]): string {
      return clsx_withTransform({
        args,
        transform: (classKey) => {
          assert(is<ClassKey>(classKey));

          return clsx(classKey, classes?.[classKey] ?? (doUseDefaultCss ? defaultClasses[classKey] : undefined));
        },
      });
    }
