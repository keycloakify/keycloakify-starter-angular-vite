...
assert<
  Extract<Extract<KcContext, { profile: unknown }>, { pageId: 'register.ftl' }> extends KcContextLike ? true : false
>();
  ...
  function getErrors(params: {
    ...
    handle_multi_valued_multi_fields: {
      ...
      required_field: {
        ...
        errors.push({
          advancedMsgArgs: ['error-user-attribute-required' satisfies MessageKey_defaultSet] as const,
          fieldIndex: undefined,
          source: {
            type: 'required field',
          },
    ...
    validator_x: {
      ...
      if (max && value.length > parseInt(`${max}`)) {
        errors.push({
          advancedMsgArgs: ['error-invalid-length-too-long' satisfies MessageKey_defaultSet, `${max}`] as const,
          fieldIndex: undefined,
          source,
        });
      }
      ...
      if (min && value.length < parseInt(`${min}`)) {
        errors.push({
          advancedMsgArgs: ['error-invalid-length-too-short' satisfies MessageKey_defaultSet, `${min}`] as const,
          fieldIndex: undefined,
          source,
        });
      }
    ...
    validator_x: {
      ...
      const msgArgs = [errorMessageKey ?? ('shouldMatchPattern' satisfies MessageKey_defaultSet), pattern] as const;
    ...
    validator_x: {
      ...
      if (max && intValue > parseInt(`${max}`)) {
        errors.push({
          advancedMsgArgs: ['error-number-out-of-range-too-big' satisfies MessageKey_defaultSet, `${max}`] as const,
          fieldIndex: undefined,
          source,
        });
      ...
      if (min && intValue < parseInt(`${min}`)) {
        errors.push({
          advancedMsgArgs: ['error-number-out-of-range-too-small' satisfies MessageKey_defaultSet, `${min}`] as const,
          fieldIndex: undefined,
          source,
        });
        break validator_x;
